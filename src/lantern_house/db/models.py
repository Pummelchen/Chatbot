...
class Relationship(Base):
    ...
    last_shift_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, nullable=False)
...
class SceneState(Base):
    ...
    current_hour_bucket: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class Message(Base):
    ...
    tick_no: Mapped[int] = mapped_column(Integer, nullable=False)
    scene_id: Mapped[int | None] = mapped_column(ForeignKey("scene_state.id"), nullable=True)
    speaker_character_id: Mapped[int | None] = mapped_column(ForeignKey("characters.id"), nullable=True)
...
class ThoughtPulse(Base):
    ...
    content: Mapped[str] = mapped_column(Text, nullable=False)
    source_directive_id: Mapped[int | None] = mapped_column(ForeignKey("manager_directives.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=_utcnow, nullable=False)
...
class Summary(Base):
    ...
    __table_args__ = (UniqueConstraint("summary_window", "bucket_end_at", name="uq_summary_window_bucket"),)
...
class RunState(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    runtime_key: Mapped[str] = mapped_column(String(50), unique=True, nullable=False, default="primary")
    status: Mapped[str] = mapped_column(String(20), nullable=False, default="idle")
    last_tick_no: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    last_public_message_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_manager_run_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_recap_hour: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    last_thought_pulse_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
