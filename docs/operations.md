...
- The manager also receives programming-grid, season-plan, monetization, canon-court, world-tracking, viewer-signal, broadcast-asset, load, and ops signals so it can trade off story ambition against runtime health.
...
- Tune daily/weekly/season tentpoles, viewer-signal polling, world-tracking thresholds, multi-candidate selection, broadcast packaging, load thresholds, canon-court behavior, monetization packaging, and ops dashboard rules in `config.example.toml`
